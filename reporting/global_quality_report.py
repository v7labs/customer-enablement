...
def validate_arguments(
    output_dir: Path, filename: str, team_slug: str, headers: Dict[str, str]
) -> None:
    """
    ...
    if not output_dir.is_dir():
        create_dir = input(
            f"The directory {output_dir} does not exist. Do you want to create it and continue? (y/n)\n"
        )
    ...
    if not filename.endswith(".csv"):
        raise ValueError(
            'Expected filename to end with ".csv"\n(example: annotator_report.csv)'
        )
...
def generate_reports(
    team_slug: str, datasets: List[Dict[str, str]], headers: Dict[str, str]
) -> Dict[str, str]:
    """
    ...
    for dataset in datasets:
        ...
        if response.status_code != 201:
            raise ValueError(
                f'Unable to generate report for dataset: {dataset["slug"]}'
            )
        else:
            print(
                f'Successfully started report generation for dataset: {dataset["slug"]}'
            )
...
def get_report_data(
    team_slug: str, report_ids: Dict[str, str], headers: Dict[str, str]
) -> List[List[str]]:
    """
...
def save_report_to_csv(
    report_data: List[List[str]], output_dir: Path, filename: str
) -> None:
    """
