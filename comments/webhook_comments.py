...
- 1: Returns comments from webhook responses
...
def get_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "folder_path", help="Path to folder containing files with comments to upload"
    )
...
def main() -> None:
    ...
    for root, dirs, files in os.walk(folder_path):
        for ix, file in enumerate(files):
            ...
            webhook_dict[webhook_body["item"]["source_info"]["item_id"]] = webhook_body[
                "item"
    ...
    for d_id, team_slug in webhook_dict.items():
        ...
        if res.status_code == 200:
            for thread in json.loads(res.text):
                ...
                url = (
                    f"https://darwin.v7labs.com/api/v2/teams/{team_slug}/items/{d_id}/comment_threads/"
                    f"{thread_id}/comments"
                )
                response = requests.get(url, headers=headers)
                print(
                    f"list comments: {'Success' if res.status_code == 200 else 'Failed'}"
                )
                comments_list.append(json.loads(response.text))

    # Dumps the comments from the webhook response into a file
    with open(
        f"./darwin comments {datetime.now().strftime('%d-%m-%Y %Hh%Mm')}.json", "w"
    ) as f:
